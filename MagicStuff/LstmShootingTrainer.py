...
if (sys.argv[3] == "TRUE"):
    net =  NetworkReader.readFrom('LstmShootingNetwork.xml')
else:
    net = buildNetwork(inputSize, inputSize, outputSize, 
                   hiddenclass=SigmoidLayer,  outclass = SigmoidLayer,  outputbias=False, recurrent=True)
...
NetworkWriter.writeToFile(net, 'LstmShootingNetwork.xml')
